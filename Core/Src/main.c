...
void TaskAFunc(void const * argument);
void TaskBFunc(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(TaskA, TaskAFunc, osPriorityNormal, 0, 128);
  TaskAHandle = osThreadCreate(osThread(TaskA), NULL);

  /* definition and creation of TaskB */
  osThreadDef(TaskB, TaskBFunc, osPriorityNormal, 0, 128);
...
/* USER CODE BEGIN Header_TaskAFunc */
/**
  * @brief  Function implementing the TaskA thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_TaskAFunc */
void TaskAFunc(void const * argument)
{
  /* USER CODE BEGIN 5 */
  int local_sumA = 0;
  for(int i = 0; i < 200; i++){
    local_sumA += data_array[i];
  }
  if(osMutexWait(myMutex01Handle, osWaitForever) == osOK) {
    total_sum += local_sumA;
    taskA_ready = 1;
...
/* USER CODE BEGIN Header_TaskBFunc */
/**
* @brief Function implementing the TaskB thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_TaskBFunc */
void TaskBFunc(void const * argument)
{
  /* USER CODE BEGIN TaskBFunc */
